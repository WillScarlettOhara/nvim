...
vim.api.nvim_buf_create_user_command(0, "GoIfErr", function()
  ...
  local result =
    vim.fn.system(string.format("iferr -pos %d", vim.fn.line2byte(pos[2]) + pos[3] - 1))
...
vim.api.nvim_buf_create_user_command(0, "GoAddTags", function(opts)
  ...
  local result = vim.fn.system(
    string.format("gomodifytags -file %s -line %d -add-tags %s", filepath, line, tags)
  )
  if vim.v.shell_error == 0 then
    vim.cmd("edit!")
...
vim.api.nvim_buf_create_user_command(0, "GoRemoveTags", function(opts)
  ...
  vim.cmd("silent write")
  local result = vim.fn.system(
    string.format("gomodifytags -file %s -line %d -remove-tags %s", filepath, line, tags)
  )
  if vim.v.shell_error == 0 then
    vim.cmd("edit!")
      ...
      vim.system({ "go", "install", tool.url }, { text = true }, function(result)
        vim.schedule(function()
          if result.code == 0 then
            ...
            notify(
              "Failed to install " .. tool.name .. ": " .. (result.stderr or ""),
              vim.log.levels.ERROR
            )
          end
        end)
